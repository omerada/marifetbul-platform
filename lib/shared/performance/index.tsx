...
export class PerformanceMonitor {
  ...
  trackExecutionTime<T>(name: string, fn: () => T): T {
    ...
    try {
      ...
      const duration = performance.now() - start;
      this.trackMetric(`${name}_execution_time`, duration, 'ms', { error: true });
      throw error;
    }
  }

  // Track async function execution time
  async trackAsyncExecutionTime<T>(name: string, fn: () => Promise<T>): Promise<T> {
    const start = performance.now();
    try {
  ...
  private trackNavigationTiming(): void {
    ...
    this.trackMetric('dns_lookup', navigation.domainLookupEnd - navigation.domainLookupStart, 'ms');
    this.trackMetric('tcp_connection', navigation.connectEnd - navigation.connectStart, 'ms');
    this.trackMetric('ssl_negotiation', navigation.connectEnd - navigation.secureConnectionStart, 'ms');
    this.trackMetric('ttfb', navigation.responseStart - navigation.requestStart, 'ms');
    this.trackMetric('response_time', navigation.responseEnd - navigation.responseStart, 'ms');
    this.trackMetric('dom_processing', navigation.domContentLoadedEventStart - navigation.responseEnd, 'ms');
    this.trackMetric('resource_loading', navigation.loadEventStart - navigation.domContentLoadedEventEnd, 'ms');
    this.trackMetric('total_load_time', navigation.loadEventEnd - navigation.navigationStart, 'ms');
  }
...
export function usePerformanceMonitor() {
  ...
  return {
    trackMetric: (name: string, value: number, unit: string, context?: Record<string, unknown>) => {
      monitor.current?.trackMetric(name, value, unit, context);
    },
    trackExecutionTime: <T>(name: string, fn: () => T) => {
      return monitor.current?.trackExecutionTime(name, fn) ?? fn();
    },
    trackAsyncExecutionTime: async <T>(name: string, fn: () => Promise<T>) => {
  ...
  return function PerformanceTrackedComponent(props: T) {
    ...
    useEffect(() => {
      if (renderStartTime.current) {
        ...
        trackMetric(`${componentName}_render_time`, renderTime, 'ms');
      }
    });
...
export function checkPerformanceBudget(budgets: Record<string, number>) {
  ...
  Object.entries(budgets).forEach(([metricName, budget]) => {
    ...
    if (metric && metric.value > budget) {
      violations.push(`${metricName}: ${metric.value}${metric.unit} exceeds budget of ${budget}${metric.unit}`);
    }
  });
