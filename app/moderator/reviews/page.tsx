 ...
 * ================================================
 * Unified review moderation interface using admin components
 * Refactored from 615 lines to use shared Sprint 2 components
 *
 * @author MarifetBul Development Team
 * @version 4.0.0 - Sprint 20 - Unified Moderation
 */
 ...
 * - Responsive layout
 * - Uses shared admin components for consistency
 */
export default function ModeratorReviewsPage() {
  const router = useRouter();
  const searchParams = useSearchParams();

  // Get initial tab from URL query params (default to flagged for moderators)
  const initialTab = (searchParams?.get('status') as TabType) || 'flagged';
  ...
  const handleTabChange = (tab: TabType) => {
    ...
    router.push(`/moderator/reviews?${params.toString()}`, {
      scroll: false,
    });

    logger.info('Moderator tab changed', { tab });
  };
  ...
  const handleReviewUpdated = () => {
    ...
    setSelectedReview(null);
    logger.info('Reviews refreshed by moderator');
  };
