...
const IYZICO_ERROR_MESSAGES: Record<string, string> = {
  // Card errors
  '10051': 'Kart numaras� ge�ersiz',
  '10005': 'Kart�n son kullanma tarihi ge�ersiz',
  '10012': 'G�venlik kodu (CVV) ge�ersiz',
  '10041': 'Kart limitiniz yetersiz',
  '10042': 'Kart limiti a��ld�',
  '10053': 'Kart�n�z �evrimi�i i�lemlere kapal�',

  // 3D Secure errors
  '10047': '��leminiz 3D Secure do�rulamas� gerektirmektedir',
  '10048': '3D Secure do�rulama ba�ar�s�z',

  // Bank errors
  '10084': '��leminiz banka taraf�ndan reddedildi',
  '10093': '��lem tekrar edilemez',

  // General errors
  '10000': 'Genel bir hata olu�tu',
...
function getErrorMessage(
  error: unknown,
  fallback = '�deme i�lemi ba�ar�s�z oldu'
): string {
  ...
  const initiatePayment = useCallback(
    async (data: PaymentIntentData): Promise<PaymentResult> => {
      ...
      if (processingRef.current) {
        ...
        return {
          ...
          error: createPaymentError(
            'validation_error',
            new Error('�deme i�lemi devam ediyor')
          ),
        };
      }
      ...
      try {
        ...
        if (!response) {
          throw new Error('�deme olu�turulamad�');
        }
    ...
    async (data: ConfirmPaymentData): Promise<PaymentResult> => {
      if (processingRef.current) {
        ...
        return {
          ...
          error: createPaymentError(
            'validation_error',
            new Error('�deme onay� devam ediyor')
          ),
        };
      }
      ...
      try {
        ...
        if (!response) {
          throw new Error('�deme onaylanamad�');
        }
  ...
  const checkStatus = useCallback(
    async (paymentId: string): Promise<PaymentResult> => {
      ...
      try {
        ...
        if (!response) {
          throw new Error('�deme bilgisi al�namad�');
        }
