            ...
            <div className="flex-1">
              ...
              <div className="text-muted-foreground mt-2 flex items-center gap-3 text-xs">
                ...
                {alert.expiresAt && (
                  <>
                    <span>�</span>
                    <span>Biti�: {formatDate(alert.expiresAt)}</span>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="bg-muted/50 rounded-lg p-3 text-sm">
            <div className="flex items-start gap-2">
              ...
              <div className="space-y-1">
                <p className="mb-1 text-xs font-medium">�neriler:</p>
    ...
    return (
      <Card className={className}>
        <CardContent className="py-8 text-center">
          ...
          <h3 className="mb-2 text-lg font-medium">G�venlik Durumu �yi</h3>
          <p className="text-muted-foreground">
            �u anda herhangi bir g�venlik uyar�s� bulunmuyor.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium">G�venlik Uyar�lar�</h3>
        {filteredAlerts.length > 0 && (
          <Badge variant="outline">{filteredAlerts.length} uyar�</Badge>
        )}
      </div>
      ...
      {securityAlerts && securityAlerts.length > maxAlerts && (
        <div className="text-center">
          <Button variant="outline" size="sm">
            T�m Uyar�lar� G�r�nt�le ({securityAlerts.length - maxAlerts} daha)
          </Button>
        </div>
      )}
    </div>
