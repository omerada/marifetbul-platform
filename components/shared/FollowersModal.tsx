 ...
 * ================================================
 * Modal displaying list of followers with infinite scroll
 *
...
import { useFollowersList } from '@/hooks/business/useFollowersList';
...
/**
 * Modal for displaying followers list with search and infinite scroll
 *
 * @example
 * ```tsx
 * <FollowersModal
...
export function FollowersModal({
  ...
  const filteredFollowers = followers.filter((follower) => {
    ...
    return (
      ...
      `${follower.firstName} ${follower.lastName}`.toLowerCase().includes(query)
    );
  });
      ...
      >
        ...
        <div className="flex items-center justify-between border-b p-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">
              {username ? `${username}'in Takipçileri` : 'Takipçiler'}
            </h2>
          </div>
          <UnifiedButton
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {error && (
            <div className="text-destructive py-8 text-center">
              <p>Takipçiler yüklenirken hata oluştu</p>
              <UnifiedButton
                onClick={refetch}
                variant="outline"
                size="sm"
          ...
          {!error && filteredFollowers.length === 0 && !isLoading && (
        ...
        {pagination && pagination.total !== undefined && (
          <div className="text-muted-foreground border-t p-4 text-center text-sm">
            {pagination.total} takipçi gösteriliyor
          </div>
        )}
      </div>
    </Dialog>
