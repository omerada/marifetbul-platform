...
export function TicketDetail({ ticketId }: TicketDetailProps) {
  ...
  const handleCloseTicket = async () => {
    try {
      ...
      logger.error(
        'Ticket kapat�l�rken hata', error instanceof Error ? error : new Error(String(error)));
    }
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex-1">
            <div className="mb-3 flex items-center gap-3">
              ...
              <Badge className={getStatusColor(ticket.status)}>
                {ticket.status === 'open' && 'A��k'}
                {ticket.status === 'in_progress' && '��lemde'}
                {ticket.status === 'closed' && 'Kapal�'}
              </Badge>
              <Badge className={getPriorityColor(ticket.priority)}>
                {ticket.priority === 'high' && 'Y�ksek'}
                {ticket.priority === 'medium' && 'Orta'}
                {ticket.priority === 'low' && 'D���k'}
              </Badge>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 text-sm text-gray-600 md:grid-cols-3">
              ...
              <div>
                <span className="font-medium">Olu�turulma:</span>{' '}
      ...
      <Card className="p-6">
        <h2 className="mb-4 text-lg font-semibold">Talep Detay�</h2>
