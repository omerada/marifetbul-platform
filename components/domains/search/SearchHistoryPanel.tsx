...
export function SearchHistoryPanel({
  ...
  const handleClearAll = () => {
    if (
      window.confirm('Tüm arama geçmişini silmek istediğinizden emin misiniz?')
    ) {
      clearSearchHistory();
      loadHistory();
  ...
  const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (success) {
          loadHistory();
          alert('Arama geçmişi başarıyla içe aktarıldı.');
        } else {
          alert('İçe aktarma başarısız oldu. Dosya formatını kontrol edin.');
        }
      } catch (error) {
        logger.error(
          'Import failed',
        ...
        <div>
          ...
          {history.length === 0 ? (
            <div className="text-muted-foreground py-8 text-center">
              ...
              <p className="mt-1 text-sm">Aramalarınız burada görünecek</p>
            </div>
          ) : (
            <div className="space-y-2">
