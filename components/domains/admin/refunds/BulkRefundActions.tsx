  ...
  const handleBulkApprove = async () => {
    ...
    try {
      ...
      if (response.failedCount === 0) {
        toast.success('Toplu Onay Ba�ar�l�', {
          description: `${response.approvedCount} iade talebi onayland�. Toplam: ?${response.totalAmountApproved?.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}`,
          duration: 5000,
        });
      } else if (response.approvedCount > 0) {
        toast.warning('K�smi Ba�ar�', {
          description: `${response.approvedCount} onayland�, ${response.failedCount} ba�ar�s�z oldu.`,
          duration: 7000,
        });
      } else {
        toast.error('Toplu Onay Ba�ar�s�z', {
          description: `${response.failedCount} iade talebi onaylanamad�.`,
          duration: 7000,
        });
      }
      ...
      toast.error('Hata', {
        description:
          'Toplu onaylama s�ras�nda bir hata olu�tu. L�tfen tekrar deneyin.',
      });
    } finally {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">
              {selectedCount} iade talebi se�ildi
            </span>
          </div>
        ...
        {showNotesInput && (
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Toplu onay i�in bir not ekleyin..."
              rows={3}
            />
          </div>
        )}
