...
const EXPORT_COLUMNS: ExportColumn[] = [
  { key: 'id', label: 'Kullan�c� ID', required: true },
  ...
  { key: 'verificationStatus', label: 'Do�rulama Durumu' },
  { key: 'createdAt', label: 'Kay�t Tarihi' },
...
export function UserExportButton({
  ...
  const formatStatus = (status: string): string => {
    const statusMap: Record<string, string> = {
      active: 'Aktif',
      inactive: 'Pasif',
      suspended: 'Ask�ya Al�nm��',
      banned: 'Yasaklanm��',
      pending_verification: 'Do�rulama Bekliyor',
    };
  ...
  const handleExport = async () => {
    try {
      ...
      if (dataToExport.length === 0) {
        showError(
          'D��a Aktarma Hatas�',
          'D��a aktar�lacak kullan�c� bulunamad�'
        );
        return;
      }
      ...
      showSuccess(
        'Ba�ar�l�',
        `${dataToExport.length} kullan�c� CSV dosyas�na aktar�ld�`
      );
      ...
      showError('Hata', 'Dosya olu�turulurken bir hata olu�tu');
    } finally {
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-muted space-y-1 rounded-md p-3">
              <p className="text-sm">
                <span className="font-medium">D��a aktar�lacak:</span>{' '}
                {hasSelection ? (
                  <>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">S�tunlar� Se�in</label>
              ...
              <p className="text-muted-foreground text-xs">
                {selectedColumns.length} s�tun se�ildi
              </p>
            </div>
          </div>
