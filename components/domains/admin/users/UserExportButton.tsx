...
const EXPORT_COLUMNS: ExportColumn[] = [
  { key: 'id', label: 'Kullanıcı ID', required: true },
  ...
  { key: 'verificationStatus', label: 'Doğrulama Durumu' },
  ...
  const handleExport = async () => {
    try {
      ...
      if (dataToExport.length === 0) {
        showError(
          'Dışa Aktarma Hatası',
          'Dışa aktarılacak kullanıcı bulunamadı'
        );
        return;
      }
      ...
      showSuccess(
        'Başarılı',
        `${dataToExport.length} kullanıcı CSV dosyasına aktarıldı`
      );
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-muted space-y-1 rounded-md p-3">
              <p className="text-sm">
                <span className="font-medium">Dışa aktarılacak:</span>{' '}
                {hasSelection ? (
                  <>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Sütunları Seçin</label>
