...
export function OrderMessagingPanel({
  ...
  const otherUser =
    userRole === 'buyer'
      ? { id: order.sellerId, name: order.sellerName || 'Sat�c�' }
      : { id: order.buyerId, name: order.buyerName || 'Al�c�' };
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      try {
        ...
        showErrorToast('Mesajlar y�klenemedi');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    try {
      ...
      if (!activeConversationId) {
        try {
          ...
          showErrorToast(
            'Konu�ma ba�lat�lamad�',
            'L�tfen tekrar deneyin veya sayfay� yenileyin.'
          );
          return;
        }
      }
    ...
    <Card className={cn('flex h-[600px] flex-col', className)}>
      ...
      <div className="border-b p-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold">{otherUser.name} ile Mesajla�ma</h3>
            <p className="text-muted-foreground text-sm">
              Sipari� #{order.orderNumber}
            </p>
          </div>
          <div className="text-muted-foreground text-xs">
            {order.packageTitle || order.jobTitle || '�zel Sipari�'}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {isLoading ? (
          ...
          <div className="flex h-full items-center justify-center text-center">
            <div>
              <p className="text-muted-foreground mb-2">Hen�z mesaj yok</p>
              <p className="text-muted-foreground text-sm">
                �lk mesaj� g�ndererek sohbeti ba�lat�n
              </p>
            </div>
          </div>
        ) : (
