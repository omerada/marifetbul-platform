...
export const RefundStatsWidget: React.FC<RefundStatsWidgetProps> = ({
  ...
  // Error state
  if (error) {
    logger.error('RefundStatsWidget: Error loading stats', error instanceof Error ? error : new Error(String(error)));
    return (
      <div className={className}>
        <Alert variant="destructive">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            �statistikler y�klenirken bir hata olu�tu. L�tfen sayfay� yenileyin.
          </AlertDescription>
        </Alert>
      </div>
    );
